...
function gen_puzzle_from_data(data) {
	...
	let quaternion_vertex_shader = document.getElementById( 'vertexShader' ).textContent;
	let quaternion_fragment_shader = document.getElementById( 'fragmentShader' ).textContent;
	...
	for (let i = 0; i < data.panels.length; ++i) {
		...
		if (panel.color < 0) {
			...
			quaternion_materials[i] = new THREE.ShaderMaterial(
				{
					uniforms: {
						cubie_center: { value: new THREE.Vector3(panel.cubie_center[0], panel.cubie_center[1], panel.cubie_center[2]) },
					},
					vertexShader: quaternion_vertex_shader,
					fragmentShader: quaternion_fragment_shader,
				}
			);
		}
	}
